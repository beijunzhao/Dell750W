<template>
	<view class="container">
		...
		<view class="action-section">
			<!-- 未进入校准模式 -->
			<button v-if="!calMode" class="action-btn enter-btn" :disabled="!connected" @tap="enterCalibration">
				进入校准模式
			</button>
...
<script>
	...
	const TARGET_VOLTAGES = [0.0, 1.0, 2.5, 5.0, 10.0, 12.0]
	// 按住连续发送间隔 (ms)
	...
	export default {
		data() {
			return {
				connected: false,
				targetVoltages: TARGET_VOLTAGES,
				calMode: false,
				calStep: 1,
				calTarget: 0.0,
				calAdc: 0,
		...
		onShow() {
			...
			bleService.onData((data) => {
				...
				if (data.cal_step !== undefined) {
					...
					this.calTarget = TARGET_VOLTAGES[this.calStep - 1] || 0
				}
		...
		methods: {
			...
			async enterCalibration() {
				...
				try {
					...
					this.calMode = true
					this.calStep = 1
					this.calTarget = TARGET_VOLTAGES[0]
					this.calResults = []
